...
async function startScan(mode) {
  ...
  // Derive the real total from the loaded (cached) word list so the counter
  // and progress bar match what the scanner actually iterates, even if a
  // list's length changes. Falls back gracefully if the list fails to load.
  const serial = fileLoadSerial
  let total = 0
  try { total = (await scanner.loadList(listKey)).length } catch { /* scan() surfaces the error */ }
  // The await above can outlive the state it was started for: a reset or new
  // file swaps the handler, a rapid 10k/100k toggle supersedes this list
  // choice, and a successful unlock leaves the locked state entirely.
  // Bail out rather than scan a discarded PDF or stale list.
  if (serial !== fileLoadSerial ||
      handler !== state.securityHandler ||
      listKey !== scanListKey[mode] ||
      state.current !== mode) return
  const fpBtn     = isOwner ? ownerForgotPasswordBtn : forgotPasswordBtn
  const choices   = isOwner ? ownerScanChoices       : scanChoices
...
function transition(newState) {
  // Leaving a locked state means the password is no longer being sought:
  // stop any running dictionary scan so it cannot burn CPU after unlock or
  // mutate the now-hidden locked-panel inputs via onFound.
  if (newState !== state.current &&
      (state.current === 'locked' || state.current === 'owner-locked')) {
    scanner.cancel()
  }
...
function resetToIdle() {
  ...
  // Discard the previous file's recovered passwords and scan results
  resetScanUI('locked')
...
async function saveBytes(bytes, filename) {
  ...
  if ('showSaveFilePicker' in window) {
    try {
      ...
      // User cancelled the save dialog — do not force a download
      if (err.name === 'AbortError') return null
      console.warn('showSaveFilePicker failed, falling back to download:', err)
      // API failure — fall through to traditional download
    }
  }
...
function handleFileSelected(file) {
  const error = validateFile(file?.type ?? '', file?.size ?? 0, file?.name ?? '')
  if (error) { fileInput.value = ''; alert(error); return }
...
async function loadPDF(bytes) {
  // Capture the load serial so a reset or a newer file selection that occurs
  // while PDFSession.load is awaiting cannot clobber the current idle/other state.
  const serial = fileLoadSerial
  try {
    ...
    // A stale rejection must not wipe a newer file that loaded meanwhile
    if (serial !== fileLoadSerial) return
...
async function attemptUnlock(password) {
  ...
  try {
    ...
    // Distinguish a genuine wrong-password from a corrupt/unreadable file so
    // the user is not sent into a futile password-retry loop on a real failure.
    const isAuthErr = /password|decrypt|crypt/i.test(err?.message ?? '')
    if (!isAuthErr) console.warn('Unlock failed (non-auth error):', err)
    unlockErrorText.textContent = isAuthErr
...
unlockForm.addEventListener('submit', e => {
  e.preventDefault()
  if (unlockBtn.disabled) return  // guard against double-submit while a prior unlock is in flight
...
function updateCommonPwWarning(password, warningEl) {
  ...
  if (!_commonPwLoading) {
    ...
    loadCommonPasswords().then(() => {
      ...
      _commonPwLoading = false  // load failed — retry on the next call
    })
  }
...
async function applyProtection() {
  // Trim once at the boundary so the values compared below are exactly the
  // values handed to protect(), which trims before encrypting.
  const userPw  = userPassword.value.trim()
  const confirmPw = userPasswordConfirm.value.trim()
  const ownerPw = ownerPassword.value.trim()
  const ownerConfirmPw = ownerPasswordConfirm.value.trim()
  const algo    = ENCRYPTION_ALGO
  const hasUserPw = userPw.length > 0
  ...
  // Capture before the awaits: Start over during async work resets state
  const session = state.session
  const fileName = state.fileName

  try {
    const bytes = await session.protect({
    ...
    const filename = outputFilename(fileName, 'protected')
    const result = await saveBytes(bytes, filename)
    if (!result || state.session !== session) return
    userPassword.value = ''
    userPasswordConfirm.value = ''
    ownerPassword.value = ''
    ownerPasswordConfirm.value = ''
...
async function doRemoveProtection(ownerPw) {
  ...
  try {
    const bytes = await session.removeProtection(ownerPw || null)
    if (state.session !== session) return
    const filename = outputFilename(fileName, 'unlocked')
    const result = await saveBytes(bytes, filename)
    if (!result || state.session !== session) return
    showSaveResult(result)
    ownerPwRow.hidden = true
